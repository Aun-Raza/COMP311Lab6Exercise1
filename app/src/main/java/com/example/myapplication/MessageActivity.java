...
public class MessageActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        deliveredPI = PendingIntent.getBroadcast(this, 0, new
                Intent(DELIVERED), PendingIntent.FLAG_IMMUTABLE | PendingIntent.FLAG_UPDATE_CURRENT);        // intent to filter the action for SMS messages received
            ...
            public void onReceive(Context arg0, Intent arg1) {
                switch (getResultCode()) { // Retrieve the current result code, as set by the previous receiver
                    case Activity.RESULT_OK:
                        Toast.makeText(getBaseContext(), getResources().getString(R.string.msg_sms_sent), Toast.LENGTH_LONG).show();
                        break;
                    case SmsManager.RESULT_ERROR_GENERIC_FAILURE:
                        Toast.makeText(getBaseContext(), getResources().getString(R.string.msg_generic_failure), Toast.LENGTH_SHORT).show();
                        break;
                    case SmsManager.RESULT_ERROR_NO_SERVICE:
                        Toast.makeText(getBaseContext(), getResources().getString(R.string.msg_no_service), Toast.LENGTH_SHORT).show();
                        break;
                    case SmsManager.RESULT_ERROR_NULL_PDU:
                        Toast.makeText(getBaseContext(), getResources().getString(R.string.msg_null_pdu), Toast.LENGTH_SHORT).show();
                        break;
                    case SmsManager.RESULT_ERROR_RADIO_OFF:
                        Toast.makeText(getBaseContext(), getResources().getString(R.string.msg_radio_off), Toast.LENGTH_SHORT).show();
                        break;
                }
            }
        };
        ...
        smsDeliveredReceiver = new BroadcastReceiver(){
            ...
            public void onReceive(Context arg0, Intent arg1) {
                switch (getResultCode()) {
                    case Activity.RESULT_OK:
                        Toast.makeText(getBaseContext(), getResources().getString(R.string.msg_sms_delivered), Toast.LENGTH_LONG).show();
                        break;
                    case Activity.RESULT_CANCELED:
                        Toast.makeText(getBaseContext(), getResources().getString(R.string.msg_sms_not_delivered), Toast.LENGTH_LONG).show();
                        break;
                }
            }
        };
